		...
		for (jStep=0;jStep<viewport.h;jStep++) {
			...
			if (dist<=radius) {
				...
				for (int i = 0; i< numPointLights; i++) {
					...
					float dotOfReflectiveViewer= pow(max(reflectiveZ, (float) 0), powerCoefficient);
				...
				for (int i = 0; i< numPointLights; i++) {
					...
					pointLightY = pointLightY / pointLightTotal;
					pointLightZ = pointLightZ / pointLightTotal;
					float dotProductResult = max((float) 0, pointLightX * normalX + pointLightY * normalY + pointLightZ * normalZ);
				...
				for (int i = 0; i< numDirectionalLights; i++) {
					...
					directionLightY = directionLightY / directionLightTotal;
					directionLightZ = directionLightZ / directionLightTotal;
					float dotProductResult = max((float) 0, directionLightX * normalX + directionLightY * normalY + directionLightZ * normalZ);
...
int main(int argc, char *argv[]) {   // first argument is the program running
	...
	while (i < argc) {
		...
		if (lineArg == "-ka" || lineArg == "-kd" || lineArg == "-ks") {
			...
			float redValue;
            istringstream(redString.c_str()) >> redValue;
            redValue = redValue * 255;

			i++;
			string greenString = argv [i];
			float greenValue;
            istringstream(greenString.c_str()) >> greenValue;
            greenValue = greenValue * 255;

			i++;
			string blueString = argv [i];
			float blueValue;
            istringstream(blueString.c_str()) >> blueValue;
			...
			float powerValue;
            istringstream(powerString.c_str()) >> powerValue;
